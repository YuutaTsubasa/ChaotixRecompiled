...
**時機**：請求是在模式分派器套用的，而標題畫面要「自己跑完」才會回到那裡 ——
注入一次 Start 只會跳過開場動畫，停在 PUSH START，之後還要等標題自己逾時，實測約 **513 幀（8.5 秒）**，
玩家看到的就是「按了 START 卻先進到 START 頁面」。
因此前端在請求還沒被取走之前會**持續輕點 Start**（按 6 幀、放 18 幀，以模擬幀計數，否則一次模擬多幀的批次會把輕點拉成長按），
取走後立刻停止（否則那個 Start 會在關卡裡變成暫停）。實測降到 **26–74 幀**。
測試 `stage_select_starts_the_chosen_level` 同時檢查「進到指定關卡」與「200 幀內完成」。

**一輪何時結束**：遊戲沒有「時間挑戰」的概念，跑完（或時間到、或任何原因）就直接把玩家送去大廳。
實測讓關卡時間耗盡：**模式仍是 `0x18`，但 `$FFDFF2` 從 0 變成 7（WORLD ENTRANCE）**。
因此前端的判定是「zone 或 level 不再是當初要求的那一組」，見 `src/frontend/time_attack.{h,cpp}`。
判定成立時把成績記下來、把前端叫回來（否則玩家就被丟在大廳）。到達關卡時間上限（36000 幀）的那一輪不算成績。

**計時器（原本認錯了）**：`$FFE052` 曾被當成關卡計時器，它不是。
它確實在開始遊玩時歸零，而且先前每次對照 HUD 都吻合——但**那些對照全都是「進關後沒有再按任何鍵」的跑法**。
按下任何鍵它就歸零，HUD 卻繼續走：實測一次跳躍讓它從 888 變回 1，同時 HUD 從 `0'14"58` 走到 `0'16"41`。
它其實是「距離玩家上次操作的幀數」（大概是用來決定何時開始播 attract demo）。
HUD 畫的是關卡引擎自己的幀計數器 `$FFE002`（long）減去「開始計時那一刻的值」。
定位那一刻仍然靠 `$FFE052`：它在整個關卡入場演出期間都是 0，而玩家在取得控制權之前無法按任何鍵把它重置，
所以基準值在它第一次非 0 的那一幀取得，之後不再使用它。
以「每兩秒跳一次」的跑法對照 HUD 五個時間點全部完全吻合（HUD 比計數器慢 5 幀）。

**但這樣還是會多算。** `$FFE002` 在「遊戲自己的時鐘停住」時仍然繼續走：暫停時 HUD 凍結、`$FFE002` 照跑。
使用者回報一輪成績多了約 25 秒，就是關卡結束後的結算動畫被算進去了。
遊戲把「時鐘停住的幀數」記在 **`$FFAEEC`**：關卡進行中完全不動，跨過一次暫停就剛好前進該次暫停的長度（實測暫停 600 幀、它前進 600）。
因此一輪的計時是「計數器的增量減去 `$FFAEEC` 的增量」，而且**逐幀以差值累加** ——
`$FFAEEC` 起始值接近 65535，只要幾秒的停止時間就會繞回，用絕對值會算錯。
驗證：暫停 600 幀期間我們的時鐘與 HUD 一起凍結在同一個值，恢復後兩者完全一致（測試 `time_attack_does_not_count_a_pause`）。

**跑到終點時時鐘也會停，而那不是 `$FFAEEC` 記的那種停止**：遊戲把完成時間留在結算畫面上，
計數器卻繼續跑完二十幾秒的結算動畫 —— 使用者實測 0'34"66 被記成 0'53"26。
`$FFFDC5` 在「畫面上的時間停住」的那一幀由 0 變 0xFF，正常遊玩全程 0，關卡因時間到而結束時也維持 0。
（另外 `$FFFDE8`、`$FFC256` 同時同值變化。）
顯示的時間會在旗標之後再走 4 幀才定住，所以停錶時要補上這個實測差。
只有「在這一輪之中看到它由 0 變起來」才算數，避免上一輪殘留的旗標讓下一輪一開始就停錶。

**起算點**：一輪從哪裡開始計時，沒有任何變數直接記著（`$FFAEFE` 看起來像，但不是 —— 在一整輪 TECHNO TOWER 裡它一直保持著上一關的值）。
能用的是 `$FFE052`：它從開始遊玩起每幀加一，所以**在它沒被碰過的期間，它本身就是經過時間**，計數器減掉它就是起點。
但不能直接讀：它會帶著上一個場景的殘值進來（那一輪帶著 77），要到入場動畫中途才被歸零；
而且歸零之後還會有**單獨一幀跳成 1** 再回到 0。
所以只有「連續數幀每幀加一」的讀數才算數，並取所有合格讀數推得**最早**的起點 ——
玩家按下任何鍵會把它歸零，而那只會讓推得的起點變晚，不會變早。
實測：TECHNO TOWER 那一輪原本早了 414 幀（約 7 秒），修正後與 HUD 完全一致。

**怎麼找到的**：前端會把每一輪的輸入錄下來（`SaveData/last_run.txt`），`chaotix_headless --replay-input` 可以原封不動重播，
而且錄製檔帶著當時的設定與結果，重播會自己回報 `faithful` 還是 `DRIFTED`。
拿一輪真的跑到終點的錄製，比對「終點前穩定、終點後穩定、剛好在那一幀改變」的位元組，
再用一段普通遊玩的重播把「正常也會出現該值」的候選刷掉，剩下的就是它。
`tests/recordings/finished_run.txt` 就是那一輪，測試 `time_attack_matches_what_the_player_saw`
會重播它並要求結果正好是玩家當時看到的 0'34"66。
...
| 關卡「跑完」的訊號（用於記錄最佳成績） | ✅ 已驗證：遊戲把玩家送去大廳（zone → 7），見 §8.3 |
| 關卡計時器 | ✅ 已驗證：`$FFE002` long 減去開始時的值；`$FFE052` **不是**計時器（見 §8.3 與 docs/ACHIEVEMENTS.md） |
